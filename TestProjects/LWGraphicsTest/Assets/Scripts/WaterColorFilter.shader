    ...
    SubShader
    {
        ...
        Pass 
        {
            HLSLPROGRAM
                ...
                struct v2f_paper {
				    float2 texcoord_Main : TEXCOORD0;
				    float2 texcoord_Paper : TEXCOORD1;
				    float4 vertex : SV_POSITION;
			    };
                
                v2f_paper VertPaper(AttributesDefault v) {              
                    float aspect = _ScreenParams.x / _ScreenParams.y;
                    v2f_paper o;
                    o.vertex = float4(v.vertex.xy, 0.0, 1.0);                    
                    o.texcoord_Main = TransformTriangleVertexToUV(v.vertex.xy);
                        ...
                        o.texcoord_Main = o.texcoord_Main * float2(1.0, -1.0) + float2(0.0, 1.0);
                    #endif                  
                    o.texcoord_Paper = o.texcoord_Main * float2(aspect, 1) * _PaperScale;
                    return o;
			    }
			
                float4 FragPaper(v2f_paper i) : SV_Target {
                    float4 src = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, i.texcoord_Main);
                    float paper = SAMPLE_TEXTURE2D(_PaperTex,sampler_PaperTex, i.texcoord_Paper).x;
                    float d = _PaperPower * (paper - 0.5) + 1;
                    return ColorMod(src, d);
                }
                #pragma vertex VertPaper
                #pragma fragment FragPaper
            ENDHLSL
        }
    }
}
